...
- (void)loadImageThumbnailsFromAlbum:(NSString *)albumName withCallback:(LoadImagesCallback)callback {
	__block NSMutableArray <NZAssetImageFile> *items = [@[] mutableCopy];
	...
	void (^assetGroupEnumerator)(ALAssetsGroup *, BOOL *) = [ ^(ALAssetsGroup *group, BOOL *stopGroup) {
	    ...
	    dispatch_group_async(dgroup, queue, ^{
	            ...
	            [group enumerateAssetsUsingBlock:[ ^(ALAsset *result, NSUInteger index, BOOL *stopAsset) {
	                        if (result) {
	                            // Get thumbnail and attach it to array
	                            NZAssetImageFile *file = [[NZAssetImageFile alloc] init];

	                            file.image = [UIImage imageWithCGImage:[result thumbnail]
	                                                             scale:1
	                                                       orientation:0];
...
- (void)loadLargeImageFromPath:(NSString *)assetPath withCallback:(SaveImageCallback)callback {
	...
	ALAssetsLibraryAssetForURLResultBlock resultblock = ^(ALAsset *myasset)
	{
		...
		if (iref) {
			callback([UIImage imageWithCGImage:iref], nil);
		}
	};
